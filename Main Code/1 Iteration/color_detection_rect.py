...
def name_contour(closing,frame_contours):
    ...
    """
    # contours
    contours, hierarchy = cv.findContours(closing, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)[-2:]
    cnt = contours[0]
    #cv.drawContours(frame_contours,contours,-1,(0,255),3)
    x,y,w,h = cv.boundingRect(cnt)
    frame_contours = cv.rectangle(frame_contours,(x,y),(x+w,y+h),(0,255,0),10)

    # put text in the image identifying the color
    cv.putText(frame_contours, hsv_color, (x, y),font,3,(0,255,0),5) 
...
# read file
with open('/home/alegria/VCI/OPENCV_COURSE/color_rect.json') as json_file:
    data = json_file.read()
